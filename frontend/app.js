...
async function applyRequestedLandingView() {
  const requested = sessionStorage.getItem("sujaanNavTarget");
  if (!requested) return;
  sessionStorage.removeItem("sujaanNavTarget");
...
async function boot() {
  ...
  await applyRequestedLandingView();
  renderAccount();
